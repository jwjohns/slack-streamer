...
describe("renderText", () => {
  ...
  it("prepends italicized status", () => {
    expect(renderText("Content", { status: "Thinking..." })).toBe("_Thinking..._\nContent");
  });

  it("trims whitespace from status", () => {
    expect(renderText("Content", { status: "  Loading...  " })).toBe("_Loading..._\nContent");
  });
  ...
  it("works with empty content", () => {
    expect(renderText("", { status: "Thinking..." })).toBe("_Thinking..._\n");
  });

  it("preserves multiline content", () => {
    expect(renderText("Line 1\nLine 2", { status: "Status" })).toBe("_Status_\nLine 1\nLine 2");
  });
});
